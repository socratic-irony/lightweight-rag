            ...
            while remaining and len(selected) < k:
                ...
                for i in remaining:
                    ...
                    for j in selected:
                        ...
                        sim = len(doc_terms[i] & doc_terms[j]) / (
                            len(doc_terms[i] | doc_terms[j]) + 1e-9
                        )
...
def apply_diversity_selection(
    ...
    for chunk_idx, score in results:
        ...
        if chunk_idx >= len(corpus):
            print(
                f"Warning: chunk index {chunk_idx} out of range (corpus size: {len(corpus)}). Skipping."
            )
            continue
